...
def plot_fitted_garage(
    garage, data, xlim=(-2.5, 2.5), ylim=(-2.5, 2.5), figsize=(10, 10)
):
    d = [data, garage.corners, garage.waypoints]  # data
    labels = ["Fitted data", "Garage corners", "Garage waypoints"]  # labels
    walls = np.array(
        [
            [
                garage.corners[0, 2],
    ...
    ax = plot_data(d, labels, walls, xlim=xlim, ylim=ylim, figsize=figsize)
