...
class Start(State):
    ...
    def get_next_state(self):
        return self.automat.states["Task2"] if self.automat.memory.button_pressed else self.automat.states["Start"]
...
class Scan_T2(State):
    ...
    def action(self):
        ...
        for angle in angles:
            ...
            # Scan environment
            yellow_points = self.automat.turtle_vision.sample_garage(color="yellow",
                    sampling_step = 1, minimal_size=YELLOW_MIN_SIZE)
            purple_points = self.automat.turtle_vision.sample_garage(color="purple",
                    sampling_step = 1, minimal_size=PURPLE_MIN_SIZE)
    ...
    def get_next_state(self):
        ...
        TurtleUtils.plot_fitted_garage(
            ...
            TurtleUtils.robot2plt_numpy(np.copy(self.automat.turtle_map_yellow.points_filtered))
        )
...
class Pycomat(Automat):
    ...
    def reset(self):
        ...
        self.turtle_map_yellow = TurtlebotMap(filter_eps = 0.5, filter_min_samples = 12)  # Map for yellow points in images
        self.turtle_map_purple = TurtlebotMap(filter_eps = 0.1, filter_min_samples = 4)  # Map for purple points in images
