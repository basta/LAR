...
class TurtlebotMap:
    ...
    def add_points(self, points, odometry):
        ...
        points_transformed = np.array(
            TurtleUtils.transform_points(points, position, yaw)
        ).T
    ...
    def clusters(self):
        if self._points is None:
            return []
        dbscan = DBSCAN(eps=self.filter_eps, min_samples=self.filter_min_samples).fit(
            self._points.T
        )
    ...
    def points_filtered(self):
        ...
        most_common_label = sorted(
            list(label_counter.items()), key=lambda x: x[1], reverse=True
        )[0][0]
        filtered_points = self._points.T[
            np.where(dbscan.labels_ == most_common_label)
        ].T

        # Remove any clusters far away from the origin
        filtered_points = filtered_points[
            :, np.linalg.norm(filtered_points, axis=0) < 3.39
        ]
