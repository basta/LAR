...
class TurtlebotICP:
            ...
            for i in range(2):
                ...
                for left, back, right in itertools.product([0, 1], repeat=3):
                    ...
                    parameters = {
                        "rotation": angle,
                        "degrees": True,
                        "translation": np.zeros(shape=(2, 1)),
                        "left": left,
                        "right": right,
                        "back": back,
                        "N": data.shape[1] + extra_points,
                        "height": 0.49 - 0.07,
                        "width": 0.59 - 0.07,
                    }
                    ...
                    # Optimize
                    P_values, cost, corresp_values = self.icp_least_squares(
                        garage, data
                    )
    ...
    def icp_least_squares(self, garage_model, Q):
        ...
        # Total cost
        cost_g = (
            closest_point(garage_model.sampled, Q[:, i]) ** 2 for i in range(Q.shape[1])
        )
        cost = sum(cost_g)
...
class GarageModel:
    def __init__(self, params):
        ...
        # |
        # LB, RB, LF, RF
        self.corners = np.array([[0.0, WIDTH, 0.0, WIDTH], [0.0, 0.0, HEIGHT, HEIGHT]])
        ...
        # 0. Garage mid-points
        self.gmp = np.array(
            [np.mean(self.corners[0, :]), np.mean(self.corners[1, :])]
        ).reshape(2, 1)
    ...
    def _sample_garage(self, params):
        ...
        side_pts = [
            params["N"] // n_sides + (1 if x < params["N"] % n_sides else 0)
...
class Optimum:
    ...
    def animate_results(self, xlim=(-2, 2), ylim=(-2, 2), save_file=None):
        ...
        anim = animation.FuncAnimation(
            fig, animate, frames=len(self.P_values), interval=500, blit=True
        )
