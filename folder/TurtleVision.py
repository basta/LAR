...
class TurtlebotVision:
    ...
    def get_regions(self, color = "yellow", minimal_size = 10):
        ...
        return TurtleUtils.mask_regions(mask, minimal_size = minimal_size)
    ...
    def sample_garage(self, color = "yellow", sampling_step = 20, r2p = False):
        regs = self.get_regions(color = color)
        depth_img_strip = get_depth_image_strip_filtered()
        points = list()
        ...
        points = TurtleUtils.robot2pyplot(points) if r2p else points
        return points
    ...
    def garage_entry_waypoints(self):
        ...
        # Left pillar pixel
        p1 = 0.5 * (regs[0][0] + regs[0][1])
        
        # Right pillar pixel
        p2 = 0.5 * (regs[1][0] + regs[1][1])
